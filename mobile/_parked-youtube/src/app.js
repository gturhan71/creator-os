...
const STRINGS = {
  ...
  "yt.title": { tr: "YouTube", en: "YouTube" },
  "yt.desc": { tr: "Kanalını bağla; yorumlar Gelen Kutusu'na gelsin ve uygulamadan cevaplayabil. Uygulama yalnızca yorumları okur ve yanıtlar; Google girişin bu cihazda kalır.", en: "Connect your channel to bring comments into the Inbox and answer them from the app. The app only reads and replies to comments; your Google sign-in stays on this device." },
  "yt.connectBtn": { tr: "YouTube'u Bağla", en: "Connect YouTube" },
  "yt.proRequired": { tr: "YouTube entegrasyonu Pro özelliğidir – Profil'den yükselt", en: "YouTube integration is a Pro feature – upgrade in Profile" },
  "yt.unavailable": { tr: "YouTube bu sürümde henüz kurulmadı.", en: "YouTube isn't set up in this build yet." },
  "yt.connectedAs": { tr: "Bağlı kanal", en: "Connected channel" },
  "yt.lastSync": { tr: "Son eşitleme", en: "Last sync" },
  "yt.never": { tr: "henüz yok", en: "not yet" },
  "yt.syncBtn": { tr: "Şimdi eşitle", en: "Sync now" },
  "yt.syncing": { tr: "Eşitleniyor…", en: "Syncing…" },
  "yt.disconnectBtn": { tr: "Bağlantıyı kes", en: "Disconnect" },
  "yt.connected": { tr: "YouTube bağlandı", en: "YouTube connected" },
  "yt.disconnected": { tr: "YouTube bağlantısı kesildi", en: "YouTube disconnected" },
  "yt.newComments": { tr: "{n} yeni yorum alındı", en: "{n} new comments imported" },
  "yt.noNew": { tr: "Yeni yorum yok", en: "No new comments" },
  "yt.syncFailed": { tr: "YouTube eşitlemesi başarısız", en: "YouTube sync failed" },
  "yt.reconnect": { tr: "YouTube erişimin sona erdi – tekrar bağlan", en: "YouTube access has expired – connect again" },
  "yt.quota": { tr: "YouTube'un günlük sınırı doldu – yarın tekrar dene", en: "YouTube's daily limit is used up – try again tomorrow" },
  "yt.note": { tr: "Yeni yorumlar uygulamayı açtığında veya Eşitle'ye dokunduğunda alınır. YouTube uygulamaya bildirim gönderemez, bu yüzden uygulama kapalıyken bir şey gelmez.", en: "New comments are fetched when you open the app or tap Sync. YouTube can't push to the app, so nothing arrives while it is closed." },
  "detail.sendYoutube": { tr: "YouTube'da Yanıtla", en: "Reply on YouTube" },
  "detail.openYoutube": { tr: "YouTube'da Aç", en: "Open on YouTube" },
  "detail.youtubeReplied": { tr: "Yanıt YouTube'da yayınlandı", en: "Reply posted on YouTube" },
  "detail.youtubeSending": { tr: "Gönderiliyor…", en: "Posting…" },
  "route.YOUTUBE_COMMENT": { tr: "YouTube yorumu", en: "YouTube comment" },
...
const state = {
  ...
  profile: null,
  accounts: [],
  youtube: { connected: false, channelTitle: "", syncing: false, error: null },
};
...
function renderThreadDetail() {
  ...
  el.innerHTML = `
    ...
    ${
      th.aiDraftText != null
        ? `
      <div class="ai-block-label">${t("detail.aiSuggestion")} <span class="confidence-tag">${t("detail.confidence")}: %${Math.round(th.confidence * 100)}</span></div>
      ...
      ${
        awaiting
          ...
          : `${
            ...
            <div class="detail-actions">
              <button class="btn btn-primary" id="btn-send">${th.channel === "EMAIL" ? t("detail.sendEmail") : th.channel === "YOUTUBE_COMMENT" ? t("detail.sendYoutube") : t("detail.sendInstagram")}</button>
              ${th.channel === "YOUTUBE_COMMENT" && th.externalUrl ? `<button class="btn btn-ghost" id="btn-open-yt">${t("detail.openYoutube")}</button>` : ""}
  ...
  async function sendYouTubeReply(text) {
    if (!entitlements.isPro()) return showToast(t("yt.proRequired"), "info");
    const btn = document.getElementById("btn-send");
    btn.disabled = true;
    btn.textContent = t("detail.youtubeSending");
    try {
      await youtube.replyToComment(th.externalId, text);
    } catch (err) {
      btn.disabled = false;
      btn.textContent = t("detail.sendYoutube");
      if (err.kind === "auth") await refreshYouTubeState();
      showToast(youtubeErrorText(err), "error");
      return;
    }
    try {
      await markReplied(text);
      showToast(t("detail.youtubeReplied"), "success");
    } catch (err) {
  ...
  async function startSend(viaDm) {
    ...
    if (!text) return;
    if (th.channel === "YOUTUBE_COMMENT") return sendYouTubeReply(text);
    try {
  ...
  // Shared by "I sent it" (mail/Instagram) and the direct YouTube reply: saves
  // the final text, moves the thread to "replied" and counts any link bundles
  // whose links ended up in the sent text.
  ...
  // Unlike Instagram/e-mail, YouTube lets the app post the reply itself.
  document.getElementById("btn-open-yt")?.addEventListener("click", () => openExternal(th.externalUrl, { newTab: true }));
  ...
  el.innerHTML = `
    ...
    <div>
      ...
      <div class="perf-stat" style="margin-top:10px;">
        <div class="perf-value">%${d.performance.engagementRate}</div>
...
function renderAccountsList() {
  ...
  el.querySelectorAll("[data-remove-account]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        if (state.accounts.find((a) => a.id === btn.dataset.removeAccount)?.channel === "YOUTUBE_COMMENT") {
          await disconnectYouTube(); // also revokes the Google sign-in and clears its tokens
          return;
        }
...
const YT_MAX_IMPORT = 25; // each imported comment gets classified and drafted (an OpenAI call for Pro users)
const YT_FIRST_SYNC_DAYS = 14;

function youtubeErrorText(err) {
  if (err?.kind === "auth") return t("yt.reconnect");
  if (err?.kind === "quota") return t("yt.quota");
  return `${t("yt.syncFailed")}: ${err?.message ?? err}`;
}

async function refreshYouTubeState() {
  const session = await youtube.getSession();
  state.youtube.connected = Boolean(session);
  state.youtube.channelTitle = session?.channelTitle ?? "";
}
...
function renderYouTube() {
  const el = document.getElementById("youtube-section");
  if (!el) return;
  const yt = state.youtube;
  let body;
  if (!youtube.isConfigured()) {
    body = `<div class="manage-limit-note">${t("yt.unavailable")}</div>`;
  } else if (!entitlements.isPro()) {
    body = `<div class="manage-limit-note">${t("yt.proRequired")}</div>`;
  } else if (!yt.connected) {
    body = `<div class="pro-desc">${t("yt.desc")}</div>
      <div class="pro-actions"><button class="btn btn-primary" id="btn-yt-connect">${t("yt.connectBtn")}</button></div>`;
  } else {
    const synced = state.settings.youtube_synced_at;
    body = `<div class="manage-count">${t("yt.connectedAs")}: <strong>${esc(yt.channelTitle)}</strong></div>
      <div class="manage-count">${t("yt.lastSync")}: ${synced ? formatRelativeOrTime(synced) : t("yt.never")}</div>
      <div class="pro-actions">
        <button class="btn btn-primary" id="btn-yt-sync" ${yt.syncing ? "disabled" : ""}>${yt.syncing ? t("yt.syncing") : t("yt.syncBtn")}</button>
        <button class="btn btn-ghost" id="btn-yt-disconnect">${t("yt.disconnectBtn")}</button>
      </div>
      <div class="pro-desc">${t("yt.note")}</div>`;
  }
  el.innerHTML = `<div class="pro-card"><div class="pro-title">${t("yt.title")}</div>${body}${yt.error ? `<div class="manage-limit-note">${esc(yt.error)}</div>` : ""}</div>`;

  document.getElementById("btn-yt-connect")?.addEventListener("click", async () => {
    yt.error = null;
    try {
      await youtube.beginSignIn();
    } catch (err) {
      showToast(youtubeErrorText(err), "error");
    }
  });
  document.getElementById("btn-yt-sync")?.addEventListener("click", () => syncYouTube());
  document.getElementById("btn-yt-disconnect")?.addEventListener("click", disconnectYouTube);
}

async function finishYouTubeSignIn(url) {
  try {
    const { channelTitle } = await youtube.completeSignIn(url);
    if (!state.accounts.some((a) => a.channel === "YOUTUBE_COMMENT" && a.connectionState === "CONNECTED")) {
      await db.createAccount({ channel: "YOUTUBE_COMMENT", handle: channelTitle });
    }
    await refreshYouTubeState();
    state.youtube.error = null;
    await refreshAccountsUi();
    showToast(t("yt.connected"), "success");
    renderYouTube();
    await syncYouTube({ silent: true });
  } catch (err) {
    state.youtube.error = youtubeErrorText(err);
    showToast(state.youtube.error, "error");
    renderYouTube();
  }
}
...
async function disconnectYouTube() {
  ...
  for (const a of state.accounts.filter((acc) => acc.channel === "YOUTUBE_COMMENT")) await db.deleteAccount(a.id);
  for (const key of ["youtube_last_sync", "youtube_synced_at"]) {
  ...
  await refreshAccountsUi();
  renderYouTube();
  showToast(t("yt.disconnected"), "info");
}

// Pulls new comments into the inbox. No push exists for YouTube, so this runs
// when the app opens/resumes and on the Sync button — never in the background.
async function syncYouTube({ silent = false } = {}) {
  const yt = state.youtube;
  if (yt.syncing || !entitlements.isPro() || !yt.connected) return;
  yt.syncing = true;
  yt.error = null;
  renderYouTube();
  try {
    const cursor = state.settings.youtube_last_sync || new Date(Date.now() - YT_FIRST_SYNC_DAYS * 86_400_000).toISOString();
    const comments = await youtube.fetchNewComments({ sinceIso: cursor });
    const fresh = [];
    for (const c of comments) {
      if (!(await db.hasThreadWithExternalId("YOUTUBE_COMMENT", c.externalId))) fresh.push(c);
    }
    const batch = fresh.slice(-YT_MAX_IMPORT); // the newest ones, oldest-first
    for (const c of batch) {
      const th = await db.createThread({
        channel: "YOUTUBE_COMMENT",
        sender: c.author,
        senderHandle: c.authorChannelId,
        fullMessage: c.text,
    ...
    const newest = comments.at(-1)?.publishedAt;
    if (newest) await saveSetting("youtube_last_sync", newest);
    await saveSetting("youtube_synced_at", new Date().toISOString());
    if (batch.length) {
      state.threads = await db.getThreads();
      renderThreadList();
      renderChannelBar();
    }
    if (!silent) showToast(batch.length ? t("yt.newComments", { n: batch.length }) : t("yt.noNew"), batch.length ? "success" : "info");
  } catch (err) {
    yt.error = youtubeErrorText(err);
    if (err.kind === "auth") await refreshYouTubeState();
    if (!silent) showToast(yt.error, "error");
  } finally {
    yt.syncing = false;
    renderYouTube();
  }
...
function handleDeepLink(url) {
  ...
  if (youtube.isRedirect(url)) {
    finishYouTubeSignIn(url);
    return;
  }
  let parsed;
  try {
