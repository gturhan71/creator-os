...
export async function init() {
  ...
  if (!hadLinkBundles) await runLinkSeed({ createLinkBundle });
  ...
  for (const stmt of [
    ...
    "ALTER TABLE thread ADD COLUMN external_id TEXT",
    "ALTER TABLE thread ADD COLUMN external_url TEXT",
  ]) {
    try {
...
export async function createThread(data) {
  ...
  await run(
    `INSERT INTO thread (id, channel, account_id, sender, sender_handle, snippet, full_message, intent, confidence, classified_by, status, unread, ai_draft_text, persona_id, received_at, external_id, external_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', 1, ?, ?, ?, ?, ?)`,
    [id, data.channel, account?.id ?? null, data.sender, data.senderHandle ?? null, snippet, data.fullMessage, classification.intent, classification.confidence, classification.source, aiDraftText, personaId, data.receivedAt ?? new Date().toISOString(), data.externalId ?? null, data.externalUrl ?? null]
  );
  return getThread(id);
}

// True when a message imported from an outside service (e.g. a YouTube
// comment) is already in the inbox — keeps repeated syncs from duplicating it.
export async function hasThreadWithExternalId(channel, externalId) {
  return Boolean(await one("SELECT id FROM thread WHERE channel = ? AND external_id = ?", [channel, externalId]));
}
